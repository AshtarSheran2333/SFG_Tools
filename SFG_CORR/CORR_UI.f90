...
#include "utils_error_macros.h"
    ...
    subroutine print_help()
        ...
        write(output_unit,f_line)   heading(flat_pattern,"Help dialog of SSP_CORR")
        ...
        write(output_unit,f_line)   "" !TODO this must be different, something like -g "NAME" -> mandatory -L, repeat
        write(output_unit,f_line)   "-L(layer) <number1 (from)> <number2 (to)>"
        write(output_unit,f_1tab)       "specifies the layers to be analyzed. Options are:"
        write(output_unit,f_2tab)           "BOTTOM | 0 1 2 3 4 5 6 7 | 15 14 13 12 11 10 9 8 | TOP"
        write(output_unit,f_line)   ""
        write(output_unit,f_line)   heading(flat_pattern,"Optional useful options")
        write(output_unit,f_line)   ""
        write(output_unit,f_line)   "-O(output) <name>"
        write(output_unit,f_1tab)       "<name> will be appended by the output file names"
        write(output_unit,f_line)   ""
        write(output_unit,f_line)   "-S(self)" !ok, but only implemented option
        write(output_unit,f_1tab)       "enables calculation of the cross correlation terms"
        write(output_unit,f_line)   ""
        write(output_unit,f_line)   "-C(cross) <real number>" !ok, but not implemented for now
        write(output_unit,f_1tab)       "enables calculation of the cross correlation terms"
        write(output_unit,f_1tab)       "<real number> is the cutoff radius for the cross correlation terms in [A]" 
        write(output_unit,f_line)   ""
        write(output_unit,f_line)   "-N(neighborlist)" !ok, but not implemented by now
        write(output_unit,f_1tab)       "enables neighbor list for for the cross correlation calculation"
        write(output_unit,f_line)   ""
        write(output_unit,f_line)   "-D(dipole_moment_and_polarizability_derivatives) <filename>" !get rid of it, just try to read a file, if not found, stick to defaults
        write(output_unit,f_1tab)       "file with dipole moment and polarizability derivatives"
        write(output_unit,f_1tab)       "file format is described in the documentation"
        write(output_unit,f_1tab)       "default values are described in the documentation"
        write(output_unit,f_line)   ""
        write(output_unit,f_line)   "-H(help)"
        write(output_unit,f_1tab)       "prints this help dialog"
        write(output_unit,f_line)   ""
        write(output_unit,f_line)   heading(wavy_pattern,"For more details see documentation")
        write(output_unit,f_line)   ""

    end subroutine print_help

    ! goes through program options and sets the program logic
    !TODO needs to be updated and decided how it should work
