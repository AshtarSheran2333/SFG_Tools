...
!TODO gets A and M of SFG unit
subroutine get_AM(sfg_unit, frame, boxdata)
    implicit none
    type(sfg_unit_type), intent(in) :: sfg_unit
    class(current_frame_type), intent(in) :: frame
    ...
    !TODO needs to be taken from the parameters database 
    real(real64), dimension(3) :: dMdRz
    real(real64), dimension(3,3) :: dAdRz

    A = 0
    M = 0
    
    !TODO loop over the chromophores
    do n = 1, size(sfg_unit%chromophores)    
        !TODO CONSTRUCT D MATRIX SUBROUTINE
        !TODO solve actor - base vector
            u = frame%positions(:,sfg_unit%chromophores(n)%actor) - frame%positions(:,sfg_unit%chromophores(n)%base)
            u = pbc_minimum_image(u, boxdata)
            r = norm2(u)
            ! z component
            D(:,3) = u / r
            !TODO no troubles expected here

        !solve base-reference vector
            scale = size(sfg_unit%chromophores(n)%references) 
            if(scale == 0) then
                !no base -> use -Z
                v = (/0.0, 0.0, -1.0/)
            else
                ...
                v = 0
                do ref = 1, scale
                    diff = frame%positions(:,sfg_unit%chromophores(n)%references(ref)) - frame%positions(:,sfg_unit%chromophores(n)%base)
            ...
            !get vz
            diff = frame%velocities(:,sfg_unit%chromophores(n)%actor) - frame%velocities(:,sfg_unit%chromophores(n)%base)
            vz = dot_product(diff,D(:,3))

            !TODO calculate the A M
            !M_R
			do i=1,3     ! do on x,y,z
				M = M + ( D(boxdata%R,i) * dMdRz(i) * vz )
			end do
		
            !A_PQ
            do i=1,3     ! do on x,y,z
                do j=1,3     ! do on x,y,z
                    A = A +( D(bd%P,i) * dAdRz(i,j) * D(bd%Q,j) * vz )
                end do
            end do
