module INSTANTANEOUS_SURFACE
    ...
    type(instantaneous_surface_structure), protected :: instasurf
    
contains
    
    subroutine instasurf_init()!(instasurf, box, vol_element)
        !prepares the mesh to be filled
    ...
    function instasurf_calculate(atom_selection, graining_len, density_threshold, pushback) result(res)
        ...
        !$OMP PARALLEL DEFAULT(NONE)&
            ...
            !$OMP REDUCTION(min:res)
		!$OMP DO
        do i=0,instasurf%n_points(1)-1
	
			do j=0,instasurf%n_points(2)-1
				...
				!probing from bottom
				do k=max(0,instasurf%bot_index(i,j)-pushback) ,instasurf%n_points(3)-1
					prev_pos = pos
				...
				!poking rod from top
				do k=min(instasurf%n_points(3)-1, instasurf%up_index(i,j)+pushback),0,-1
					prev_pos = pos
                ...
                if( .not. found_bot_interface) res = res - 2
			end do
        end do
		!$OMP END DO
		!$OMP END PARALLEL
		...
		!TODO - here we potentially can look for the neighbor points and try to interpolate the missing points
    ...
    subroutine instasurf_read()!(filetype)
        !read the instantaneous surface from a file
        stop "NOT IMPLEMENTED"
    end subroutine instasurf_read
