...
#include "utils_error_macros.h"
    ...
    subroutine init(this, x_min, x_max, n_bins, name)
        implicit none
        class(density_profile), intent(inout) :: this
        real(real64), intent(in) :: x_min, x_max
        integer(int64), intent(in) :: n_bins
        integer(int64) :: nb
        character(64), intent(in), optional :: name
        !save the parameters
